...
**Code style:** Use Python 3.12+ type hints on all public functions. Run `ruff check src/` and `ruff format src/` — fix all findings before presenting code. No `type: ignore` unless the reason is documented inline.
...
tests/            # Test code
pyproject.toml    # Project config (single source of truth)
```

**Acceptance criteria:**
- [ ] `ruff check src/ tests/` passes with zero findings
